 ...
 *  Module:    Control
 *
 *  Summary:   This is the serial I/O module for the GPS Clock
 *             Handles UART1 and UART1 I/O
 *
 *******************************************************************/
...
// UART0-1 control flags & regs (DEBUG CLI I/O)
#define RXD0_BUFF_END 40		// CLI rx buff len
#define RXD1_BUFF_END 200		// gps rx buff len
...
S8   rxd1_buff[RXD1_BUFF_END];	// rx data buffer
U8   rxd1_hptr;					// rx buf head ptr
U8   rxd1_tptr;					// rx buf tail ptr
U8   rxd1_stat;					// rx buff status
...
void initserial(void){
    ...
    rxd1_tptr = 0;
    rxd1_stat = 0;
    init_uart1(9600L);				// gps serial init
}
...
char getch00(void){
	...
	while(rxd_tptr == rxd_hptr){
		process_IO(0, 0);							// process system I/O <idle loop here>
	}
	c = rxd_buff[rxd_tptr++];						// get chr
	if(rxd_tptr == RXD0_BUFF_END){					// wrap index if needed
		rxd_tptr = 0;
	}
	return c;
}
...
U32 init_uart1(U32 baud){
	...
	SYSCTL_RCGCUART_R |= SYSCTL_RCGCUART_R1;			// enable UART1 peripheral
	SYSCTL_RCGCGPIO_R |= SYSCTL_RCGCGPIO_R4;			// enable PC clock
	GPIO_PORTC_AFSEL_R |= 0x0030L;						// connect UART1 RX & TX to PC5 & PC4
	GPIO_PORTC_PCTL_R &= 0xFF00FFFFL;					// enable UART1 function
	GPIO_PORTC_PCTL_R |= 0x00220000L;
	GPIO_PORTC_DIR_R |= 0x0020;							// txd = out
	GPIO_PORTC_DEN_R |= 0x0030;							// RXD = TXD = digital
	for(i=0;i<100000;i++);
	return config_uart1(baud);							// config UART1
}
...
U32 config_uart1(U32 baud){
	...
	IR = (uint32_t)PIOCLK * 8L;							// IR = (sysclk * 128) / (16 * baud)
	IR /= baud;											//    = sysclk * 8 /baud {integer and fractional part combined}
	IR += 1;											// add 0.5 to round off
	IR >>= 1;
//	UART1_IBRD_R &= 0xffff0000L;
	UART1_IBRD_R = (IR>>6) & 0xffff;					// store integer part
//	UART1_FBRD_R &= 0xffffffc0L;
	UART1_FBRD_R |= IR & 0x3fL;
	UART1_CC_R = UART_CC_CS_PIOSC;						// set PIOSC as UART source
	UART1_LCRH_R |= (0x0060L | UART_LCRH_PEN);			// 8 bits + odd parity
	UART1_IM_R &= 0xfffce000L;							// set UART1 to int on RX
	UART1_IM_R |= 0x0010L;
...
//-----------------------------------------------------------------------------
// getch1 checks for input @ RX1.  If no chr, return null.
//	looks for input in rxd1_buff[] which is filled in the UART1 interrupt
//-----------------------------------------------------------------------------
S16 getchr1(void){
	...
	if(rxd1_tptr != rxd1_hptr){
		c = (S16)rxd1_buff[rxd1_tptr++] & 0x00ff;					// process system I/O
...
//-----------------------------------------------------------------------------
// initbuf1 initializes the rx buffers for UART1
//-----------------------------------------------------------------------------
void initbuf1(void){
...
//-----------------------------------------------------------------------------
// tsip_buf_stat() returns the %full status of the rxd1 buffer
//-----------------------------------------------------------------------------
U8 tsip_buf_stat(void){
	U16	i;		// temps
	U8	j = rxd1_hptr;
	U8	k = rxd1_tptr;
...
//-----------------------------------------------------------------------------
// rxd1_intr() UART rx intr (CLI I/O).  Captures RX data and places into
//	circular buffer
//-----------------------------------------------------------------------------
void rxd1_intr(void){
	char c;			// temp
	...
	while(!(UART1_FR_R & UART_FR_RXFE)){			// process RXD chrs
		c = UART1_DR_R;
		rxd1_buff[ptemp++] = c;						// feed the buffer
		if(ptemp >= RXD1_BUFF_END){
			ptemp = 0;								// wrap buffer ptr
		}
		if(ptemp == rxd1_tptr){
